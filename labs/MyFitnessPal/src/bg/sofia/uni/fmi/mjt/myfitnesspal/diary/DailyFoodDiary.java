...
public class DailyFoodDiary implements FoodDiary {
    ...
    public FoodEntry addFood(Meal meal, String foodName, double servingSize) throws UnknownFoodException {
        ...
        if (foodName == null || foodName.isEmpty() || foodName.isBlank()) { //3rd bug found - empty
        ...
        if (meals.containsKey(meal)) {
            ...
            List<FoodEntry> tempFoodEntryList = new ArrayList<>();
            tempFoodEntryList.add(foodEntry);
            meals.put(meal, tempFoodEntryList);
        }
    ...
    public double getDailyCaloriesIntakePerMeal(Meal meal) {
        ...
        if (meals.containsKey(meal)) {
            foodEntries = meals.get(meal);
        } else {
            return dailyCaloriesIntakePerMeal;
        }

        for (FoodEntry foodEntry : foodEntries) {
            dailyCaloriesIntakePerMeal += foodEntry.nutritionInfo().calories() * foodEntry.servingSize(); //1 bug found
        }
